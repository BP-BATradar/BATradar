...
def get_all_tdoa_of_chunk_index_by_gcc_phat(
    ...
    for mic1, mic2, mic3, mic4 in combinations(environment.get_mics(), 4):
        ...
        audio3 = mic3.get_audio().get_audio_signal(index=chunk_index)
        audio4 = mic4.get_audio().get_audio_signal(index=chunk_index)
        ...
        # Check if all microphones have valid audio signals
        if audio1 is not None and audio2 is not None and audio3 is not None and audio4 is not None:

            # If the audio signals do not contain a dominant signal, don't compute TDoA
            if np.max(np.abs(audio1)) < threshold or np.max(np.abs(audio2)) < threshold or np.max(np.abs(audio3)) < threshold or np.max(np.abs(audio4)) < threshold:
                if debug:
                    print(
                        f"Audio signals for mics at {mic1.get_position()}, {mic2.get_position()}, {mic3.get_position()} and {mic4.get_position()} do not contain a dominant signal."
                    )
                return None
            ...
            tdoa_pair = Tdoa4(mic1=mic1, mic2=mic2, tdoa=tdoa)
